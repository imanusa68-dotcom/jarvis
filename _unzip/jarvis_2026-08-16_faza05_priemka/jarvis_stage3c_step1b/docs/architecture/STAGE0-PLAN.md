...
- ~~save_memory — фиксация «идёт ДО гейта» (намеренное поведение)~~
  ...
  **ОБНОВЛЕНО 29.08.2026, фаза 1е: ТРЕТИЙ путь в память закрыт, все три
  писателя ходят через одну дверь.** `main._update_memory_async` — фоновый
  поток, где дом дописывает что-то САМ, без просьбы владельца — обходил
  правило Г-2 двумя вызовами: `update_memory()` (факты) и
  `update_personality()` (манера речи Джарвиса). Второе хуже: правка
  личности не фиксировалась НИГДЕ, и на «почему ты стал так отвечать?»
  ответить было нечем. Две поправки к моим же прежним словам: (1) я называл
  этот путь «самым частым» — отозвано, замер дал 6 явных сохранений против
  0 само-записей, путь самый РЕДКИЙ, чинится из-за того, ЧТО пишет;
  (2) комментарий в `core/fences.py` уверял, что personality нечем записать
  («проверено грепом по политике») — верно про ИНСТРУМЕНТ, но ПИСАТЕЛЬ был,
  просто забор не видел не-инструмент. Правка: два повода
  `memory_self_write` / `personality_self_write` в `core/security.py`
  (`planner_visible=False` — иначе модель начнёт звать их сама и выйдет
  ЧЕТВЁРТЫЙ путь), оба имени в `MEMORY_TOOLS` забора, и помощник
  `_may_write_by_itself()` в `main.py` с двумя ОТДЕЛЬНЫМИ вердиктами (общий
  скрыл бы в журнале, что именно записали). Ловушка `Unknown tool` снова
  проверена живым замером ДО правки: наивный порядок отключил бы авто-запись
  молча при зелёных тестах. Замер потоков: `contextvars` границу потока не
  переходит, фоновая запись всегда выглядит как владелец — поэтому запрет
  под-агенту работает на явных вызовах, а не здесь. 16 сторожей, 10 краснели
  до правки; 10 мутаций — 9 поймано, десятая («слать в журнал значения»)
  оказалась непробиваемой по устройству (`core/gate.py:168` читает только
  имена), утверждение «найден пробел» отозвано. Дважды исправлен СТОРОЖ, а
  не код: якорь `TOOL_DECLARATIONS` ловил собственный комментарий, а подсчёт
  слова `gate` шёл по тексту вместе с комментариями — то есть страж был бы
  зелёным без единого вызова двери. Ничего не запрещено, владельца не
  спрашивают (risk=low), diff `main.py` — только добавленные строки;
