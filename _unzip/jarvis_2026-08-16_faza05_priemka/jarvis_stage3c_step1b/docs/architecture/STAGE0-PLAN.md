...
- ~~save_memory — фиксация «идёт ДО гейта» (намеренное поведение)~~
  **ОТМЕНЕНО 28.08.2026, фаза 1в, с согласия владельца.** `save_memory` теперь
  ...
  `tests/test_memory_write_through_door.py`. `forget_memory` и `recall_memory`
  через дверь НЕ проведены намеренно: их нет в политике, дверь ответит им
  `Unknown tool` (проверено мутацией — забывание умирает молча);
