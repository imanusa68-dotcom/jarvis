...
> **Разработка (2026-07):** окружение управляется uv. Установка: `uv sync` → `uv run playwright install chromium`.
> Тесты: `uv run pytest` (эталон: 153+ passed). Запуск: `uv run python main.py`.
> Миграция на MARK XXXVI: см. `docs/architecture/STAGE0-PLAN.md`.
