...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        #
        # 1. НЕ ПЕРЕНЕСЁН общий блок двери. Мимо неё ходят ТРИ инструмента
        #    памяти (save 1279, forget 1298, recall 1322), но `forget_memory` и
        #    `recall_memory` ОТСУТСТВУЮТ в политике `core/security.py`, и дверь
        #    по правилу fail-closed отвечает им `blocked: Unknown tool`.
        #    Перенос блока убил бы забывание и вспоминание НАСМЕРТЬ — и тесты
        #    остались бы зелёными, потому что эти пути у двери никто не
        #    проверяет. Владелец узнал бы голосом через неделю.
        #
        ...
        if name == "save_memory":
            try:
                ...
                # Сломавшаяся дверь — ОТКАЗ, а не «пиши дальше». Правило
                # fail-closed: неизвестно, кто просит запись в память, — значит
                # не пишем. Раньше здесь записалось бы молча.
                print("[JARVIS] gate error — refusing save_memory "
                      f"(fail-closed): {_mem_gate_err}")
                if not self.ui.muted:
                    self.ui.set_state("LISTENING")
                return types.FunctionResponse(
                    id=fc.id, name=name,
                    response={"result": "SECURITY: gate error, memory not "
                                        "written (save_memory).",
                              "silent": True},
                )
            if not _mg.allowed:
                # Сегодня сюда попадает только под-агент (забор I12/Г-3).
