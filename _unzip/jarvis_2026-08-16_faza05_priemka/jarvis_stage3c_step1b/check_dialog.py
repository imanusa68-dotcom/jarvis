...
def main() -> int:
    ...
    # 2b. ЗАПИСЬ В ПАМЯТЬ — ВИДНА? (фаза 1в, добавлено 28.08.2026)
    #
    # Прогон владельца показал ровно эту дыру: он сказал «запомни, что я не пью
    # кофе после шести», Джарвис ответил «запомнил», память записалась — а
    # журнал не вырос НИ НА СТРОКУ. Память была единственным местом в доме,
    # куда можно положить факт, не оставив следа.
    #
    # Проверка стоит здесь, а не среди заборов, потому что отвечает на вопрос
    # ВЛАДЕЛЬЦА («видно ли, что он запомнил»), а не на вопрос об устройстве.
    saves = [x for x in doors if x.get("tool") == "save_memory"]
    if saves:
        print(f"[OK  ] записей в память видно в журнале: {len(saves)}"
              "   (фаза 1в)")
        last = saves[-1]
        chain = " -> ".join(last.get("origin_chain") or []) or "нет цепочки"
        print(f"       последняя: вердикт {last.get('verdict')},"
              f" просил {chain}")
        # Сказанное вслух в журнал попадать не имеет права — только имена.
        keys = last.get("param_keys") or []
        if "said" in keys or "value" in keys:
            print("       в строке ИМЕНА параметров (said/value), а не то,"
                  " что вы сказали — так и надо")
    else:
        print("[--  ] записей в память в журнале нет — скажите «Джарвис,"
              " запомни, что ...»")
        print("       если сказали, а строки нет — правка фазы 1в не работает")
