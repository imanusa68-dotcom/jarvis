...
def test_the_description_stays_within_a_sane_size():
    ...
    ПОТОЛОК ПОДНЯТ 29.08.2026, 4000 -> 4700, и это осознанное решение, а не
    подгонка под красный тест. Смысл сторожа — «не расти МОЛЧА», а рост
    назван владельцу вслух дважды: шаг 2' (+1241 знак, тон записи и выбор
    ключа) и шаг 2'' (+651, разрешение НЕ писать и новый человек в брани).
    Прежде чем поднимать, я сократил повторы в самом описании на 107 знаков
    (три места: «Those belong to this conversation», лишнее «silently»,
    длинная формулировка про кота) — потолок поднят только на остаток.
    Если понадобится поднять В ЧЕТВЁРТЫЙ раз, это уже сигнал, что текст
    исчерпан и задачу надо решать не текстом.
    """
    assert len(DESC) <= 4700, (
