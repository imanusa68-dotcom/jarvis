...
TOOL_DECLARATIONS = [
    ...
    {
        "name": "save_memory",
        # ── ДВОЙНОЙ ОТВЕТ ЛЕЧИТСЯ ЗДЕСЬ, А НЕ ПРОСЬБОЙ В ПРОМПТЕ ──────────
        #
        # ЧТО БЫЛО. Владелец слышал ответ дважды: по существу — до вызова
        # этого инструмента, и доклад о записи — после него. Замер по его
        # журналу 29.08.2026: 3 хода из 3, где инструмент звался, и 0 из 2,
        # где не звался. Причину заказывал наш же промпт правилом «just call
        # it, then report naturally» — 7 таких правил против НУЛЯ исключений
        # для тихих инструментов памяти.
        #
        # ПОЧЕМУ ПРОМПТА НЕДОСТАТОЧНО. Инструкция — просьба, и владелец
        # прямо сказал: «я не хочу чтобы это было реже, я хочу чтобы эта
        # проблема полностью исчезла». Просьбой этого не добиться: модель
        # вправе счесть доклад «естественным ответом», как её и просят
        # остальные правила. Нужен механизм, а не уговор.
        #
        # ПОЧЕМУ NON_BLOCKING. У протокола есть штатное средство закончить
        # вызов, НЕ ВЫЗВАВ генерацию ответа: `scheduling=SILENT` вместе с
        # `will_continue=False` (ставятся в ответе, см. `_execute_tool`).
        # Документация SDK владельца (2.12.1, проверено зондом 41) говорит
        # это дословно: «To avoid triggering the generation and finish the
        # function call, additionally set `scheduling` to `SILENT`». Но там
        # же сказано: «Only applicable to NON_BLOCKING function calls, is
        # ignored otherwise» — то есть без этой строки оба поля СЕРВЕР
        # ПРОИГНОРИРУЕТ молча.
        #
        # МОЯ ПРЕЖНЯЯ ОШИБКА, ЧЕСТНО. В шапке `core/echo_guard.py` я написал,
        # что SILENT нам не подходит, «потому что все инструменты
        # блокирующие». Посылка верна, вывод — нет: `behavior` объявляем МЫ,
        # и достаточно одной строки, чтобы условие выполнилось. Правильный
        # вывод был не «нельзя», а «надо объявить». Шапка исправлена.
        #
        # ПОЧЕМУ ТОЛЬКО ЗДЕСЬ, А НЕ ВСЕМ 23 ИНСТРУМЕНТАМ. Для остальных
        # ответ инструмента И ЕСТЬ ответ владельцу: у `web_search` без
        # генерации после результата разговор оборвётся на полуслове,
        # `recall_memory` обязан озвучить найденное, а `forget_memory` —
        # сказать, забылось ли. Молчание уместно РОВНО там, где сказать
        # нечего: запись прошла, докладывать не о чем.
        #
        # ОТКАТ: убрать эту строку. Выключатель на случай, когда лезть в код
        # не хочется — `"JARVIS_SILENT_MEMORY_WRITE": false` в
        # ~/.jarvis/settings.json; он снимает и SILENT в ответе.
...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name == "save_memory":
            ...
            if not self.ui.muted:
                self.ui.set_state("LISTENING")
            # ── ВТОРАЯ РЕПЛИКА ОТМЕНЯЕТСЯ ЗДЕСЬ ──────────────────────────
            #
            # Пара к `"behavior": "NON_BLOCKING"` в объявлении save_memory.
            # Объявление разрешает серверу принять эти два поля, а они
            # закрывают вызов, НЕ ВЫЗВАВ генерацию ответа. Без объявления
            # сервер их молча проигнорирует, без этих полей объявление
            # ничего не меняет — работают только вместе.
            #
            # ПОЧЕМУ РОВНО ЗДЕСЬ, А НЕ ВЫШЕ И НЕ НИЖЕ. Это единственная
            # точка, где запись УЖЕ СОСТОЯЛАСЬ. Двумя ответами выше стоят
            # отказы двери (сломанная дверь и запрет забора) — им молчание
            # ставить НЕЛЬЗЯ: память не тронута, и владелец обязан это
            # услышать. Немой отказ памяти хуже двойного ответа: двойной
            # раздражает, немой — обманывает, потому что владелец уйдёт
            # уверенным, что факт сохранён.
            #
            # `silent: True` в словаре оставлен намеренно: он был здесь до
            # правки, на него смотрят сторожа
            # (tests/test_memory_write_through_door.py:86), и убирать чужой
            # контракт заодно с правкой — прятать одно в другом. Модели он,
            # впрочем, ничего не говорит: самодельное поле, протокол его не
            # знает, смысл нигде не объяснён (замерено зондом 39). Настоящую
            # работу делают `scheduling` и `will_continue`.
            #
            # ОТКАТ: `"JARVIS_SILENT_MEMORY_WRITE": false` в
            # ~/.jarvis/settings.json — и ответ снова обычный, как до правки.
            _extra = {}
            try:
                from core.feature_flags import silent_memory_write_enabled
                if silent_memory_write_enabled():
                    _extra = {"scheduling": "SILENT", "will_continue": False}
            except Exception as _flag_err:
                # Сломавшийся выключатель не имеет права ломать запись:
                # факт уже на диске. Тихо возвращаемся к прежнему поведению
                # (двойной ответ), а не падаем посреди хода.
                print(f"[Memory] выключатель тишины недоступен, ответ "
                      f"обычный: {_flag_err}")
            return types.FunctionResponse(
                id=fc.id, name=name,
    ...
    async def _receive_audio(self, stop: asyncio.Event):
        ...
        out_buf: list[str] = []
        in_buf:  list[str] = []
        # ── ГРАНИЦА ХОДА: СКОЛЬКО СКАЗАНО ДО ВЫЗОВА ИНСТРУМЕНТА ──────────
        # Исследование 29.08.2026 (зонды 39-40). Жалоба владельца «отвечает
        # два раза» — это ДВА разных дефекта, и сторож эха видел только один:
        # дословный дубль от сервера. Второй случай — ответ по существу ДО
        # вызова и доклад о записи ПОСЛЕ него: слова разные, `looks_doubled`
        # даёт False, а ухо слышит тот же самый двойной ответ. В журнале
        # 29.08 такое было на 3 ходах из 3, где звался инструмент.
        #
        # Различить их по тексту НЕЛЬЗЯ — примёты нет. Единственный признак
        # структурный, и он проходит ровно через этот цикл: расшифровка,
        # потом `tool_call`, потом ЕЩЁ расшифровка. Раньше порядок здесь
        # терялся: `out_buf` копится от начала хода до `turn_complete`, а
        # блок `tool_call` его не трогает, поэтому обе картины давали одну
        # склеенную строку. Один int восстанавливает границу.
        #
        # Почему int, а не второй список: копия текста создала бы второй
        # источник правды о сказанном. Индекс — это отметка, а не данные.
        said_before_tool = -1
            ...
            try:
                async for response in self.session.receive():
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                ...
                                try:
                                    ...
                                    pass
                                # Вторая реплика того же хода: сказанное
                                # ПОСЛЕ ответа инструмента. Отдельный
                                # try/except по той же причине, что и выше —
                                # диагностика не имеет права убить сессию.
                                try:
                                    if 0 <= said_before_tool < len(out_buf):
                                        from core.echo_guard import (
                                            describe_split_turn as _split,
                                        )
                                        _w2 = _split(
                                            " ".join(out_buf[:said_before_tool]),
                                            " ".join(out_buf[said_before_tool:]),
                                        )
                                        if _w2:
                                            print(_w2)
                    ...
                    if response.tool_call:
                        # Отметка границы. Ставится ОДИН раз за ход: при
                        # нескольких вызовах подряд «до» — это речь до
                        # ПЕРВОГО из них, иначе вторая отметка съела бы
                        # реплику, сказанную между вызовами.
                        if said_before_tool < 0:
                            said_before_tool = len(out_buf)
                        fn_responses = []
