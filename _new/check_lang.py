...
VOICE_REFERENCE = {
    # ПЕРЕСНЯТО 29.08.2026 (второй раз за день), ШАГ 0: возврат из `finally`.
    # Предыдущее значение 3aa14d5d… (141370 Б) — фаза 1е, авто-запись через дверь.
    #
    # Причина: в `_run_session` стоял `return uptime` ВНУТРИ `finally`. Замер
    # показал, что такой возврат поглощает ЛЮБОЕ исключение, и из-за этого в
    # цикле `run` были НЕДОСТИЖИМЫ три ветки — остановка по Ctrl+C, разбор
    # ошибки и `is_fatal_error`. Живьём это значило: при неверном ключе API
    # Джарвис вечно ходил по кругу переподключений вместо остановки с внятной
    # ошибкой. Единственный вызов `is_fatal_error` стоял в мёртвой ветке.
    #
    # В main.py тронуты ТРИ места, все — рядом с переподключением: объявление
    # `_last_uptime` в `__init__`, сам `finally` в `_run_session` и развилка по
    # устойчивости в ветке ошибки. Отрисовка, синтез речи и голосовой путь НЕ
    # тронуты. В diff 40 добавленных строк против 4 удалённых, и все четыре
    # удалённые — это те же строки, переписанные на месте (`return uptime`,
    # `uptime = ...`, `guard.record_failure()`), а не убранная работа.
    #
    # Чем этот пересъём отличается от прошлых: сторож НЕ был просрочен. Он
    # honestly показал ровно одну проблему — эту, предсказанную заранее, — и
    # ни одной чужой. Прогон владельца: 1898 passed, и сумма его файла
    # (b6ca6ef7…, 144300 Б) совпала с суммой из песочницы байт в байт, то
    # есть архив дошёл без искажений. `ui.py` байт в байт прежний.
    #
    # НАЗВАННЫЙ ДОЛГ: тестов на `is_fatal_error`/`is_recoverable_error` в
    # tests/ НЕ СУЩЕСТВУЕТ. Эта ветка не была защищена ничем, и 1898 тестов
    # прошли бы и с мёртвым кодом. Правку судил замер, а не набор тестов.
    # (Долг закрыт в тот же день: tests/test_fatal_error_reaches_the_exit.py.)
    #
    # ПЕРЕСНЯТО 29.08.2026 (третий раз за день), ШАГ 6, ПОБОЧНАЯ НАХОДКА:
    # выключатели диагностики умирали от пробела из cmd.
    # Предыдущее значение b6ca6ef7… (144300 Б) — шаг 0, возврат из finally.
    #
    # Причина. Владелец запустил ровно то, что я просил, слово в слово:
    #     set JARVIS_DEBUG_PROMPT=1 && python main.py
    # Счётчик памяти напечатался, а блок — нет. Дефект не в памяти: в cmd
    # пробел ПЕРЕД `&&` попадает ВНУТРЬ значения, программа видела "1 ", а
    # строгое `== "1"` на этом молча ломается. Такая диагностика хуже
    # отсутствующей — она заставляет искать поломку в памяти, где её нет.
    #
    # В main.py тронуты ДВА места, оба — чтение переменной окружения:
    # `JARVIS_DEBUG_PROMPT` (строка ~1273) и `JARVIS_BUS_LOG` (~2340).
    # Добавлено `(... or "").strip()`. Второй выключатель правил заодно,
    # чтобы тот же дефект не выстрелил следующим. Голосовой путь, отрисовка
    # и синтез речи НЕ тронуты — ни одной строки.
    #
    # Чем этот пересъём отличается: сторож НЕ был просрочен, прогон владельца
    # чистый (1935 passed, 145 проверок), и правка не меняет ПОВЕДЕНИЕ
    # Джарвиса — только то, включается ли печать по просьбе владельца.
    # Долг закрыт сразу: tests/test_debug_switches_survive_cmd.py, 14 проверок,
    # на старом main.py падает 3 из них.
    #
    # ПЕРЕСНЯТО 29.08.2026 (четвёртый раз за день), ШАГ 1: критерий «что
    # достойно памяти» — в описании save_memory.
    # Предыдущее значение e0f6627e… (145845 Б) — шаг 6, пробел из cmd.
    #
    # Причина. В описании стоял только ЗАПРЕТ «не пиши сиюминутное», и модель
    # понимала его шире, чем нужно: «кот опять лёг на клавиатуру» — событие,
    # поэтому не писалось НИЧЕГО, хотя внутри спрятано свойство «у владельца
    # есть кот», верное и через год. Запрет без умения извлечь свойство
    # делает Джарвиса глухим.
    #
    # В main.py тронут ОДИН фрагмент — текст описания инструмента
    # save_memory (~строки 669-700). Добавлены два вопроса критерия (про
    # владельца или про мир; свойство или событие) и правило «событие может
    # обнаружить свойство» с примером про кота. Ни одной строки КОДА:
    # голосовой путь, отрисовка, синтез речи, диспетчер не тронуты.
    # Описание выросло 1407 -> 2606 знаков (+4.1% системного промпта).
    #
    # ЗАМЕР ОПРОВЕРГ ПЛАН, и текст написан по замеру: план утверждал, что
    # замена факта безопасна, потому что в схеме есть superseded_by.
    # probe26 показал — upsert_fact этот столбец НЕ заполняет, повтор пары
    # (категория, ключ) делает UPDATE и прежнее значение исчезает без следа.
    # Поэтому описание честно говорит «old value is GONE» и советует при
    # сомнении брать новый ключ. Тест сторожит и это: если upsert_fact
    # научится вести историю, тест упадёт и потребует переписать текст.
    #
    # Долг закрыт сразу: tests/test_step1_save_criterion.py, 14 проверок,
    # на старом описании падает 6 из них.
    #
    # ПЕРЕСНЯТО 29.08.2026 (шаг 2'). Тронуто ОДНО место — описание
    # save_memory, 2606 -> 3847 знаков (+1241, ещё +4.3% промпта). Голосовой
    # путь, отрисовка, синтез речи, диспетчер, память как код — не тронуты.
    # Предыдущее значение 23ee5810... (148875 Б) — шаг 1.
    #
    # Причина 1, от ВЛАДЕЛЬЦА: Джарвис отвечал как делопроизводитель —
    # «Отмечено, сэр. У вас есть друг по имени Лёха», «Обновляю информацию:
    # вашего кота зовут Лев». Разбор живого журнала (probe34): старый запрет
    # покрывал только ДЕЙСТВИЕ («не объявляй, что сохраняешь»), и модель его
    # СОБЛЮДАЛА. Запрета на пересказ РЕЗУЛЬТАТА не было нигде: он есть только
    # для блока памяти («never recite like a list»). Дефект был в тексте,
    # который я сам написал на шаге 1.
    #
    # Причина 2, ЗАМЕР ПРОТИВ ПЛАНА: план предлагал перед записью искать
    # кандидатов через search_facts. probe27-29 показали, что на ЗАМЕНЕ —
    # ровно там, ради чего это затевалось — поиск слепой: «Химки» и «BMW» не
    # делят ни одного слова с «Moscow»/«Mercedes-Benz», поиск возвращает
    # ПУСТО, и verbatim не спасает. Зато probe30-33: блок памяти уже печатает
    # «Favorite Cars: ...», и ключ восстанавливается из подписи однозначно —
    # 23 из 23. Поэтому кода в горячем пути НЕТ, есть текст, цена 0 вызовов.
    # Честная граница: выше ~85 фактов подписи выпадают из блока по бюджету.
    #
    # Долг закрыт сразу: tests/test_step2_key_reuse_and_tone.py, 18 проверок,
    # на описании до шага 2' падает 9 из них.
    #
    # ПЕРЕСНЯТО 29.08.2026 (шаг 2''). Тронуто ОПЯТЬ ОДНО место —
    # описание save_memory, 3847 -> 4498 знаков (+651, +2.2% промпта).
    # Предыдущее значение dfb0fd56... (153089 Б) — шаг 2'.
    # Голосовой путь, отрисовка, синтез, диспетчер, код памяти — не тронуты.
    #
    # ДВА ОТКАЗА ИЗ ОДНОГО ЖИВОГО ЖУРНАЛА, и они ПРОТИВОПОЛОЖНЫЕ —
    # поэтому крутить «строже/мягче» бесполезно, лечится только обоими
    # примерами сразу:
    #   1) ЛИШНЯЯ ЗАПИСЬ. «кот опять лёг на клавиатуру» -> в базу легло
    #      notes/cat_on_keyboard_event='Cat is on the keyboard again'. Ключ
    #      буквально _event — то, что шаг 1 запрещает на ЭТОМ ЖЕ примере.
    #      Замер (probe35): запрет был НА МЕСТЕ, правило модель знала.
    #      Причина была в моём же тексте шага 2': свойство «есть кот»
    #      УЖЕ лежало (cat_name=Tigr), извлекать нечего, а разрешений
    #      молчать ГОЛОСОМ было три, а разрешения НЕ ПИСАТЬ В БАЗУ —
    #      ни одного. Моя фраза «say nothing and just save» читалась как
    #      «промолчи И ЗАПИШИ». Исправлено.
    #   2) ПРОПУСК. «бл опять алекс сломал мой телефон» -> НИ ОДНОГО
    #      вызова save_memory за два хода, хотя Алекса в relationships нет,
    #      а «опять» говорит, что он в жизни владельца постоянный.
    #      Замер (probe36): пример был только РАЗРЕШАЮЩИЙ (рыбалка
    #      открывает друга); что новый ЧЕЛОВЕК — свойство даже в брани и
    #      жалобе, сказано не было.
    #
    # Цена сбита СОКРАЩЕНИЕМ, а не поднятием потолка: убраны три
    # повтора (-107 знаков) — «Those belong to this conversation» (та же
    # мысль, что «an event is over when the sentence ends»), лишнее
    # «silently» и длинная формулировка про кота.
    #
    # Долг закрыт сразу: тот же файл тестов, теперь 26 проверок,
    # на описании шага 2' падает 9 из них.
    #
    # ПЕРЕСНЯТО 29.08.2026 (пятый раз за день), ИССЛЕДОВАНИЕ ДВОЙНОГО ОТВЕТА.
    # Предыдущее значение 6216e177… (157330 Б) — шаг 2''.
    #
    # Причина. Владелец: «отвечает 2 раза, мне реально бесит». Сторож эха при
    # этом МОЛЧАЛ, и не по недосмотру: `split_echo` ловит только ДОСЛОВНЫЙ
    # дубль (случай 28.08, «меренговый торт»). Зонды 39-40 разделили жалобу
    # на ДВА несвязанных дефекта:
    #   1) серверный дубль слово в слово — не наш, звук генерирует Google;
    #   2) ответ по существу ДО вызова инструмента и доклад о записи ПОСЛЕ.
    #      Слова разные, `looks_doubled` даёт False, а ухо слышит то же самое.
    #      Замер по журналу 29.08: 3 хода из 3 с вызовом инструмента, и 0 из 2
    #      без вызова.
    #
    # Второй случай — НАШ. Его заказывал наш же промпт: замерено 7 правил вида
    # «just call it, then report» против НУЛЯ исключений для тихих
    # инструментов памяти. Наш флаг `silent: True` тут не помогает вовсе —
    # это самодельное поле внутри свободного словаря `response`, протокол его
    # не знает, и модели его смысл не объяснён нигде (замерено).
    #
    # В main.py тронуты ТРИ места, все в цикле приёма: объявление
    # `said_before_tool`, отметка границы в блоке `tool_call` и вторая
    # проверка у `turn_complete`. Причина правки — архитектурная слепота:
    # порядок событий (речь -> tool_call -> речь) проходил через наш цикл, но
    # выбрасывался при склейке `" ".join(out_buf)`, поэтому оба дефекта давали
    # ОДНУ строку и различить их было физически нечем. Отрисовка, синтез речи
    # и голосовой путь НЕ тронуты: звук к этому месту уже проигран, глушить
    # нечего — прибор только СЧИТАЕТ.
    #
    # Сторожа: tests/test_two_replies_in_one_turn.py, 14 проверок; на
    # откаченном main.py падает 9 из них.
    #
    # ── ПЕРЕСЪЁМКА 29.08.2026, ВТОРОЙ ЗАХОД: ОТМЕНА ВМЕСТО ПРОСЬБЫ ─────────
    # Владелец на первый заход ответил дословно: «Я не хочу чтобы это было
    # реже, я хочу чтобы эта проблема полностью исчезла». Возражение по делу:
    # первый заход добавил ИЗМЕРЕНИЕ (видно вторую реплику) и ПРОСЬБУ в
    # промпте, а просьбу модель вправе истолковать иначе — тем более что
    # семь других правил промпта её как раз заказывают.
    #
    ...
    # `scheduling` to `SILENT`». Условие применимости — `behavior=NON_BLOCKING`
    # («Only applicable to NON_BLOCKING function calls, is ignored otherwise»),
    # и это поле НАШЕГО объявления, то есть требование, а не запрет. Здесь же
    # исправлена моя прежняя ошибка: я писал, что SILENT нам не подходит,
    # «потому что все инструменты блокирующие» — посылка верна, вывод нет.
    #
    # В main.py тронуты ДВА места, оба в ветке одного инструмента:
    # `"behavior": "NON_BLOCKING"` в объявлении `save_memory` и
    # `scheduling=SILENT, will_continue=False` в УСПЕШНОМ ответе. Почему
    # именно так:
    #   * ровно 1 инструмент из 23 — у остальных ответ инструмента И ЕСТЬ
    #     ответ владельцу, немой web_search оборвал бы разговор;
    #   * SILENT только ПОСЛЕ состоявшейся записи — отказ двери обязан
    #     остаться слышным, иначе память не тронута, а владелец уверен, что
    #     сохранено (немой отказ хуже двойного ответа);
    #   * выключатель `JARVIS_SILENT_MEMORY_WRITE` в ~/.jarvis/settings.json
    #     возвращает прежнее поведение без правки кода;
    #   * промпт изменён В ТОН протоколу: модели велено отвечать ДО вызова,
    #     иначе SILENT съел бы единственный ответ и вышла бы тишина.
    #
    # Сторожа: tests/test_the_second_reply_is_cancelled.py, 17 проверок; на
    # откаченном main.py падает 10, и каждая половина связки ловится
    # отдельной полу-мутацией.
    # ПЕРЕСНЯTО 29.08.2026: срок годности фактов о событиях.
    # Предыдущее 283a1de4… (168070 Б).
    #
    # В main.py тронуто ОДНО место: диагностика «In prompt: N facts».
    # Она считала факты своим фильтром (_without_junk) и после ввода
    # срока годности стала бы врать ровно на скрытое. Теперь и сборка
    # промпта, и диагностика зовут одну дверь _visible_memory.
    # Голос, речь, отрисовка и переподключение НЕ тронуты.
    # ПЕРЕСНЯТО 30.08.2026: оговорка владельца — это стирание.
    # Предыдущее afab01da… (169691 Б).
    #
    # В main.py тронуто ОДНО место: описание инструмента forget_memory
    # (706 -> 1307 знаков). Причина в живом логе владельца: «я в америке
    # живу… ой тоесть Химках» — поправка легла в новый ключ, ошибочный
    # `country` остался и ехал в промпт каждую сессию.
    #
    # Описание save_memory НЕ тронуто и осталось ровно 4498 знаков: там
    # потолок 4500, и сторож это проверяет числом
    # (tests/test_a_slip_of_the_tongue_is_a_removal.py).
    # Голос, речь, отрисовка, переподключение и обработчики НЕ тронуты.
    "main.py": ("6188247b69d3113ba379d302604fae01", 173437),
...
if VOICE_REFERENCE["main.py"][0] in ("2f69e64713602327e4b3181a3ce69574",
                                     ...
                                     # эталон фазы 1е — до шага 0 (возврат из
                                     # `finally`, из-за которого `is_fatal_error`
                                     # была мёртвым кодом)
                                     "3aa14d5d74957df1f419209d90d70c90",
                                     # эталон шага 0 — до шага 6, когда
                                     # выключатели диагностики умирали от
                                     # пробела из cmd (`set VAR=1 && python`)
                                     "b6ca6ef7f3132a603c3ab677f2af6813",
                                     # эталон шага 6 — до шага 1, когда в
                                     # описании save_memory был только запрет,
                                     # без умения извлечь свойство из события
                                     "e0f6627e760a8283e5daad26a9cc0812",
                                     # эталон шага 1 — до шага 2', когда запись
                                     # озвучивалась распиской («Отмечено, сэр»)
                                     # и ключ можно было выдумать в обход блока
                                     "23ee58101ed10ea16bd169fcd4336204",
                                     # эталон шага 2' — до шага 2'', когда не было
                                     # разрешения НЕ писать в базу, и новый
                                     # человек в брани пропадал без записи
                                     "dfb0fd5600000ee5bf5451fa3d1a1d38",
                                     # эталон шага 2'' — до исследования двойного
                                     # ответа, когда граница «сказано до вызова /
                                     # после вызова» выбрасывалась при склейке и
                                     # вторая реплика в ходу была невидима
                                     "6216e17758e8394b06897256574c4db1",
                                     # эталон первого захода по двойному
                                     # ответу — когда вторую реплику отменяла
                                     # только ПРОСЬБА в промпте, без штатного
                                     # механизма протокола (NON_BLOCKING +
                                     # scheduling=SILENT)
                                     "af5612ce4b768d27e9a7d8fdab5d439a"):
...
if _stale:
    ...
    print("   эталон голоса переснят: main.py 29.08.2026 (двойной ответ: "
          "вторая реплика отменяется механизмом протокола — save_memory "
          "объявлен NON_BLOCKING, успешный ответ уходит с "
          "scheduling=SILENT), ui.py 22.08.2026")
print()
