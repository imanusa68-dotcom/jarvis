...
def _recursive_update(target: dict, updates: dict,
    ...
    for key, value in updates.items():
        ...
        # А ТЕПЕРЬ — ПОДОБРАТЬ УЖЕ ЛЕЖАЩЕЕ СТАРОЕ НАПИСАНИЕ ЭТОГО ЖЕ ФАКТА.
        #
        # ЗАМЕР 30.08.2026, ВТОРОЙ ЖИВОЙ ПРОГОН ВЛАДЕЛЬЦА. Приведение выше
        # закрыло появление НОВЫХ двойников, но НЕ вылечило уже лежащий. На
        # диске у владельца остался ключ `Cat Name` от прошлой поломки, и он
        # сказал «кота зовут лев но теперь буду звать его алексик». Журнал:
        #     forget_memory {'key':'cat_name'}                 -> Forgotten
        #     save_memory   {'key':'cat_name','value':'Alexic'} -> Saved
        # Модель сделала всё правильно. А в промпт снова уехали две строки:
        #     - Cat Name: Lev
        #     - Cat Name: Alexic
        # Потому что стирала и писала она в `cat_name`, а рядом всё это время
        # жил `Cat Name`, до которого никто не дотянулся.
        #
        # ПОЧЕМУ ЭТО НЕ «МОЛЧАЛИВАЯ ПРАВКА ЧУЖИХ ДАННЫХ». Мы не ходим по
        # памяти и не чиним её сами — это осталось запрещено. Мы трогаем
        # РОВНО тот факт, который владелец прямо сейчас перезаписывает, и
        # ровно его прежнее написание. Оставить старое имя ключа лежать
        # рядом с новым значением — это и есть потеря данных: Джарвис
        # начинает знать два имени одного кота и спрашивать, какое верное.
        #
        # ПЕРЕНОС, А НЕ ВЫБРОС: если канонического ключа ещё нет, старая
        # запись переезжает в него целиком — вместе с полем `said`, по
        # которому потом ищет поиск. Иначе исходные слова владельца
        # пропали бы при переименовании ключа.
        # ЭТА ФУНКЦИЯ РЕКУРСИВНА, И УРОВНИ ЗДЕСЬ РАЗНЫЕ. Внизу ключи —
        # ИМЕНА ФАКТОВ, наверху — КАТЕГОРИИ. Прежняя редакция этого не
        # различала, и первая же запись в `notes` УНИЧТОЖАЛА категорию
        # `Notes` целиком, со всем содержимым, молча.
        #
        # ЗАМЕР 30.08.2026, версия против версии (probe_cat):
        #     до правки e27447d:  ФАКТ ЖИВ: True
        #     после правки e92266f: ФАКТ ЖИВ: False
        # То есть дефект внесла именно предыдущая правка, а не наследие.
        # Один вызов update_memory({'notes': {...}}) при наличии на диске
        # `Notes` и `NOTES` оставлял от трёх категорий одну:
        #     до:    ['notes', 'Notes', 'NOTES']
        #     после: ['notes']
        # И это НЕ безобидно: замерено, что факты из категории в чужом
        # написании ВИДНЫ — они и в блоке промпта, и в поиске recall.
        # Значит выброс уничтожал живые, работающие факты.
        #
        # ПОЧЕМУ РАЗЛИЧАЕМ ПО ТИПУ ЗНАЧЕНИЯ, А НЕ ПО ГЛУБИНЕ РЕКУРСИИ.
        # Глубину пришлось бы протаскивать лишним доводом через все
        # вызовы, включая внешние (их в проекте несколько, и один — из
        # фонового потока). Тип же уже проверяется строкой ниже тем же
        # условием, по которому функция решает, спускаться ей или писать:
        # `isinstance(value, dict) and "value" not in value` — это
        # КОНТЕЙНЕР (категория), иначе ФАКТ. Одно решение, одно место.
        #
        # КОНТЕЙНЕР СЛИВАЕТСЯ, ФАКТ ПЕРЕЗАПИСЫВАЕТСЯ, и это не
        # непоследовательность, а разная природа данных. Два написания
        # ОДНОГО факта — противоречие, из них верно ровно одно, и
        # выбирает владелец своей последней фразой. Два написания одной
        # КАТЕГОРИИ — это просто две полки с разными фактами, и терять
        # содержимое ни одной из них нельзя.
        #
        # ПРИ СЛИЯНИИ ВЫЖИВАЕТ ТОТ, ЧТО УЖЕ В КАНОНЕ: он либо новее, либо
        # его прямо сейчас перезаписывает владелец. Старая полка отдаёт
        # только те факты, которых в канонической нет, — так ни один
        # факт не пропадает и ни один свежий не откатывается назад.
        _is_container = isinstance(value, dict) and "value" not in value
        for stale in [k for k in list(target)
                      if k != key and _canonical_key(k) == key]:
            moved = target.pop(stale)
            if key not in target:
                target[key] = moved
            elif (_is_container and isinstance(moved, dict)
                  and isinstance(target[key], dict)):
                for fact_key, fact in moved.items():
                    if fact_key not in target[key]:
                        target[key][fact_key] = fact
            changed = True
